	...
	if llama == nil {
		...
		if err != nil {
			...
			if errors.Is(err, ggml.ErrUnsupportedFormat) || strings.Contains(err.Error(), "failed to load model") {
				err = fmt.Errorf("%v: this model may be incompatible with your version of ollmlx. If you previously pulled this model, try updating it by running `ollmlx pull %s`", err, req.model.ShortName)
			}
		...
		if s.activeLoading.ModelPath() != req.model.ModelPath {
			...
			slog.Error("attempted conflicting model load", "error", err)
			req.errCh <- err
			s.loadedMu.Unlock()
			return false
		}
